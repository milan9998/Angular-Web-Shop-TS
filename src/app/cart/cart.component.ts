...
export class CartComponent implements OnInit {
  ...
  something = new MatTableDataSource<Number>(); 
  totalAmount!: number;
  clothing: Clothes[] = [];
  displayedColumns = ["id","picture","manufacturer","price","name","size", "amount", "itemtotalamount","removeFromCart"];
  displayedColumns2 = ["test"];
  clothesSource = new MatTableDataSource<Clothes>(this.clothing);
...
ngOnInit(): void {
  this.route.params.subscribe(params => {
    ...
    this.clothing = this.clothesService.getCart();
    this.clothesSource.data = this.clothing;
...
showOrderConfirmation() {
    if(this.isLoggedIn()){
        this.snackBar.open('Uspenso ste izvrsili porudzbinu!', 'Zatvori', {
            duration: 3000, 
          });
    }else{
        this.snackBar.open('Morate biti ulogovani', 'Zatvori', {
            duration: 3000, 
          });
...
calculateTotalAmount(): Number {
    this.totalAmount = this.clothing.reduce((acc, item) => {
      return acc + (item.newPrice || 0);  
    }, 0);
...
removeFromCart(element: Clothes) {
    const index = this.clothing.indexOf(element);
    
      this.clothing.splice(index, 1); 
      this.clothesSource.data = [...this.clothing]; 
      element.amount = 1;
      element.newPrice = element.price * element.amount;
      this.calculateTotalAmount();
  }
...
addToCart(product: Clothes) {
    ...
    this.clothesSource.data = this.clothing; 
    product.amount=1;
    product.newPrice = product.price * product.amount
  ...
  decreaseAmount(element: Clothes) {
    ...
    if(element.amount > 0){
        element.amount -=1 ;
        element.newPrice = element.price * element.amount;
        element.newPrice = parseFloat(element.newPrice.toFixed(2));
        this.clothesSource.data = [...this.clothing];  
    }
    this.calculateTotalAmount();
  }
  ...
  increaseAmount(element: Clothes) {
    if (element.amount === undefined) {
        element.amount = 1;
      }
    element.amount +=1 ;
